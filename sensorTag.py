...
class SensorTag:
    ...
    def __init__( self, bluetooth_adr, gdp):
        self.gdp = gdp
        self.con = pexpect.spawn('gatttool -b ' + bluetooth_adr + ' --interactive')
        result = self.con.expect('\[   \]\[' + bluetooth_adr + '\]\[LE\]>', timeout=500)
        if (result == 1):
            print 'Timeout on gatttool.'
            #sys.exit("Timeout on gatttool")
        else:
        ...
        result = self.con.expect('\[CON\]\[' + bluetooth_adr + '\]\[LE\]>', timeout=500)
        if (result == 1):
    ...
    def reqGCL(self):
        self.gcl_name = self.gdp.getGCL()
        print self.gcl_name

    def getGCL(self):
        return self.gcl_name
    ...
    def notification_loop( self ):
        while True:
            ...
            if pnum==0:
                ...
                #try:
                if True:
                    result = self.cb[handle]([long(float.fromhex(n)) for n in hxstr[2:]])
                    print result
                    self.gdp.gdpPOST(result)
                    #print "posted"
                #except:
...
def main():
    ...
    while True:
      ...
      temp = tag.char_read_hnd(0x25)
      tag.cb[0x25](temp, gdpCon)
